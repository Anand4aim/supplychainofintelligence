...
NAME: "Supply Chain of Intelligence" (no leading "The"). Trademark applied 2026-06-20. Render as "Supply Chain of Intelligence™" on first mention per piece; never prefix with "The". Readers may colloquially add "The", we do not. The Intelligence Cube™ is also a trademark of Anand Arivukkarasu.
Use SCoI (Supply Chain of Intelligence) as a subtle acronym after the trademarked name in headers, footers, posters, and metadata; keep the first mention per piece as the full "Supply Chain of Intelligence™".
...
Chess framing: framework names the square under attack (layer exposure); the juggernaut always has a counter-move. Predictions = the board, not a crystal ball. Every prediction carries a `counterMove` field. See mem://preferences/chess-framing.
...
CANONICAL DEFINITION (verbatim, evergreen, never paraphrase): "Intelligence is a supply chain. Value accrues at the bottlenecks, not the most visible node." Source of truth: src/data/definition.ts + <CanonicalDefinition />.
/framework is organized as SIX SECTIONS in this exact order: Definition (what is it?) → Map (where does value accrue?) → Laws (why does it work?) → Dynamics (how does it evolve?) → Applications (how do I use it?) → Observations (what's happening now?). Tier grouping: Substrate=L−1..L3+L8, Workflow=L4..L6, Surface=L7. Never use "Tier 0..4" or "Register" or "Knowledge Tiers" — retired. Cube and Triangle are INSTRUMENTS (section 5), never Laws.
Position SCoI as a "strategic framework for AI" — never "AI framework", never "another AI stack". Canonical reframe sentence (ships everywhere): "The AI stack explains how intelligence is built. The Supply Chain of Intelligence explains where intelligence becomes economically defensible." Lives in src/data/definition.ts as POSITIONING_LINE. The AI stack is one INPUT to SCoI, never a competitor — don't argue, reframe the category. See mem://preferences/category-reframe.

## Memories
- [Sketch poster reference](mem://design/sketch-poster-reference), Canonical hand-drawn poster style from "Anatomy of an AI Agent" image: bold marker title, Caveat annotations, rainbow colored-pencil fills on warm paper. North star for all editorial illustrations.
- [Two-register visual system](mem://design/two-register-system), editorial (sketch) vs reference (clean) surfaces; which pages use which; AuthorNote rules
